...
_DETACHED_LAUNCHER = (
    ...
    "stdout=sys.stdout, stderr=subprocess.STDOUT, close_fds=True, "
...
def _read_hook(path: Path) -> tuple[str | None, int]:
    ...
    try:
        return path.read_text(encoding="utf-8"), mode | stat.S_IXUSR
...
def install_graph_hooks(repository: Path, orichum_launcher: Path) -> None:
    ...
    for name in _HOOK_NAMES:
        ...
        updates.append((path, _updated_hook(content, name, launcher), mode))
...
def graph_hook_status(repository: Path) -> str:
    ...
    try:
        ...
        missing = False
        for name in _HOOK_NAMES:
            content, _ = _read_hook(hooks / name)
            if content is None:
                missing = True
                continue
            cleaned = _remove_marked_section(
                content, _ORICHUM_START, _ORICHUM_END
            )
            if cleaned == content:
                missing = True
...
def _safe_attributes(repository: Path) -> tuple[Path, str] | None:
    path = repository / ".gitattributes"
    content, _ = _read_hook(path)
    if content is None:
        return None
    return path, content
...
def remove_upstream_graphify_hooks(repository: Path) -> None:
    ...
    if (
        ...
        return
    attributes_path, content = attributes
    lines = content.splitlines()
    if _GRAPHIFY_ATTRIBUTE not in (line.strip() for line in lines):
        return
    kept = [line for line in lines if line.strip() != _GRAPHIFY_ATTRIBUTE]
    mode = stat.S_IMODE(attributes_path.stat().st_mode)
    if kept:
        _write_hook(attributes_path, "\n".join(kept) + "\n", mode)
    else:
        try:
...
def _launch_detached_update(
    ...
    if log.exists():
        ...
        if (
            ...
            raise GraphHookError("Graph update log is unsafe")
        if observed.st_size > _MAX_LOG_BYTES:
            if os.path.lexists(previous):
                prior = os.lstat(previous)
                if (
                    stat.S_ISLNK(prior.st_mode)
                    or not stat.S_ISREG(prior.st_mode)
                    or prior.st_uid != os.getuid()
                ):
                    raise GraphHookError("Previous graph update log is unsafe")
            try:
                os.replace(log, previous)
            except OSError as error:
                raise GraphHookError("Graph update log cannot be rotated") from error
    flags = (
        os.O_WRONLY
        | os.O_APPEND
        | os.O_CREAT
    ...
    try:
        ...
        launcher = subprocess.Popen(
            [sys.executable, "-I", "-B", "-c", _DETACHED_LAUNCHER, json.dumps(command)],
            cwd=repository,
